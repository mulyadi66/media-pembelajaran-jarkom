...
export default function GlossaryPageKKA() {
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState('Semua');

  const categories = ['Semua', ...new Set(flashcardsKKA.map(c => c.category))];
  const filtered = flashcardsKKA.filter(card => {
  ...
  return (
    <div className="content-section">
      <div className="materi-card fade-in">
        <h3><BookOpen size={20} /> Glossarium KKA</h3>
        <p style={{marginBottom: 20}}>Daftar istilah penting dalam Koding dan Kecerdasan Artifisial.</p>
        ...
        <p style={{fontSize:'0.85rem',color:'var(--text-light)',marginBottom:16}}>
          Menampilkan {filtered.length} dari {flashcardsKKA.length} istilah
        </p>
