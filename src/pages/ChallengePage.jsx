...
export default function ChallengePage() {
  const [mode, setMode] = useState(null); // null, 'ready', 'playing', 'done'
  ...
  const startGame = () => {
    const qs = shuffle(challengeQuestions).slice(0, selectedCount);
    setQuestions(qs);
    setCurrentIdx(0);
    setUserAnswer('');
    setResults([]);
    ...
    return (
      <div className="content-section">
        <div className="materi-card fade-in" style={{textAlign:'center', maxWidth: 600, margin: '0 auto'}}>
          <h3><Zap size={20} /> Latihan Cepat Subnetting</h3>
          <p style={{marginBottom: 24, color:'var(--text-light)'}}>
            Uji kecepatanmu menjawab pertanyaan subnetting! Jawab secepat mungkin untuk skor tertinggi.
          </p>
        ...
        <div style={{display:'flex', flexDirection:'column', gap:8}}>
          {results.map((r, i) => (
            <div key={i} style={{
              ...
              background: r.isCorrect ? '#ecfdf522' : '#fef2f222',
              fontSize: '0.85rem'
            }}>
