...
export default function GlossaryPage() {
  ...
  const categories = ['Semua', ...new Set(flashcardData.map(c => c.category))];

  const filtered = flashcardData.filter(card => {
  ...
  return (
    <div className="content-section">
      <div className="materi-card fade-in">
        <h3><BookOpen size={20} /> Glossarium Jaringan Komputer</h3>
        <p style={{marginBottom: 20}}>Daftar istilah penting dalam perencanaan dan pengalamatan jaringan.</p>
        ...
        <p style={{fontSize:'0.85rem',color:'var(--text-light)',marginBottom:16}}>
          Menampilkan {filtered.length} dari {flashcardData.length} istilah
        </p>
