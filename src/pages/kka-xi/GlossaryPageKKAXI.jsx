...
export default function GlossaryPageKKAXI() {
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState('Semua');

  const categories = ['Semua', ...new Set(flashcardsKKAXI.map(c => c.category))];
  const filtered = flashcardsKKAXI.filter(card => {
  ...
  return (
    <div className="content-section">
      <div className="materi-card fade-in">
        <h3><BookOpen size={20} /> Glossarium KKA XI</h3>
        <p style={{marginBottom: 20}}>Daftar istilah penting dalam Koding & Kecerdasan Artifisial XI.</p>
        ...
        <p style={{fontSize:'0.85rem',color:'var(--text-light)',marginBottom:16}}>
          Menampilkan {filtered.length} dari {flashcardsKKAXI.length} istilah
        </p>
