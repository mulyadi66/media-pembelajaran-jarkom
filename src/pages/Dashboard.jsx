...
import { Server, Projector, CreditCard, Briefcase, ClipboardCheck, FileText, BarChart3, BookOpen, Trophy, Network, Monitor } from 'lucide-react';

export default function Dashboard() {
  const { modulesRead, scores } = useApp();

  const modules = [
    { to: '/modul1', icon: Server, title: 'Peralatan Jaringan', desc: 'Kebutuhan teknis, peralatan, dan teknologi jaringan', color: ['#667eea', '#764ba2'] },
    { to: '/modul2', icon: Projector, title: 'Topologi Jaringan', desc: 'Star, bus, ring, mesh, tree, hybrid + simulasi', color: ['#f093fb', '#f5576c'] },
    { to: '/modul3', icon: CreditCard, title: 'Pengalamatan Jaringan', desc: 'IP Address, Subnetting, CIDR, VLSM + kalkulator', color: ['#4facfe', '#00f2fe'] },
  ];

  const quickActions = [
    { to: '/kasus', icon: Briefcase, title: 'Studi Kasus', desc: 'Terapkan ilmu dalam skenario nyata', color: ['#6366f1', '#818cf8'] },
    ...
    { to: '/hasil', icon: BarChart3, title: 'Hasil Penilaian', desc: 'Lihat nilai & rekomendasi', color: ['#06b6d4', '#0891b2'] },
  ];
  ...
  return (
    <>
      ...
      <section className="section-block">
        ...
        <div className="modules-grid">
          {modules.map(({ to, icon: Icon, title, desc, color }) => {
            const read = modulesRead[to.replace('/', '')];
            return (
              <Link to={to} className="module-card" key={to}>
                ...
                <div className="module-info">
                  ...
                  <div className="module-progress">
                    <div className="progress-bar"><div className="progress-fill" style={{width: read ? '100' : '0' + '%'}} /></div>
                    <span>{read ? '100%' : '0%'}</span>
                  </div>
                </div>
                <ChevronRightIcon />
              </Link>
      ...
      <footer className="footer">
        <p>&copy; 2026 JarkomLab — Media Pembelajaran TJKT</p>
      </footer>
    </>
  );
}
...
function ChevronRightIcon() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{color:'var(--text-lighter)', flexShrink: 0}}><polyline points="9 18 15 12 9 6"/></svg>;
}
