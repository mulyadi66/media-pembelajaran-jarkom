...
export default function CodecComparison() {
  ...
  const selStyle = (isSel) => ({
    background: '#2d2d44', borderRadius: 10, padding: 14,
    border: `1.5px solid ${isSel ? '#6366f1' : '#3d3d5c'}`,
  ...
  return (
    <div style={{
          ...
          return (
            <div key={c.name} style={selStyle(isSel)} onClick={() => toggle(c.name)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                <span style={{ fontSize: 16, fontWeight: 700, color: isSel ? '#fff' : '#94a3b8' }}>
                  {c.name}
                </span>
              ...
              <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 6 }}>{c.desc}</div>
      ...
      {selectedCodecs.length > 0 && (
        <div style={{
          background: '#2d2d44', borderRadius: 10, padding: 16, marginTop: 14,
        }}>
      ...
      <div style={{
        ...
        <div style={{
          background: '#2d2d44', borderRadius: 8, padding: '10px 14px', flex: 1, minWidth: 160,
        }}>
          <span style={{ color: '#94a3b8' }}>Best Quality: </span>
          <span style={{ color: '#22c55e', fontWeight: 700 }}>{bestQuality.name}</span>
          <span style={{ color: '#94a3b8' }}> (MOS {bestQuality.mos})</span>
        </div>
        <div style={{
          background: '#2d2d44', borderRadius: 8, padding: '10px 14px', flex: 1, minWidth: 160,
        }}>
          <span style={{ color: '#94a3b8' }}>Best Bandwidth: </span>
          <span style={{ color: '#22c55e', fontWeight: 700 }}>{bestBandwidth.name}</span>
          <span style={{ color: '#94a3b8' }}> ({bestBandwidth.bandwidth})</span>
        </div>
      </div>
    </div>
  );
